...
const dateFormat = (param) => {
  if (param) {
    const s = useDateFormat(param * 1000, 'YYYY-MM-DD HH:mm:ss')
    return s.value
  }
  return ""
}
...
const ship_time = computed(() => {
  if (props.info.ship_data) {
    return dateFormat(props.info.ship_data.express_time)
  }
})

// 支付时间
const paid_time = computed(() => {
  return dateFormat(props.info.paid_time)
})
